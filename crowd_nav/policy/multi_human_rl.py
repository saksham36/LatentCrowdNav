...
# class MultiHumanRL2(CADRL):
#     def __init__(self):
#         super().__init__()

#     def predict(self, state):
#         """
#         A base class for all methods that takes pairwise joint state as input to Q network.
#         The input to the Q network is always of shape (batch_size, # humans, rotated joint state length)
...
#             Q = self.model(rotated_batch_input).data
...
#     def transform(self, state):
#         """
#         Take the state passed from agent and transform it to the input of Q network
...
#         else:
#             state_tensor = self.rotate(state_tensor)
#         print(f'In multihumanRL: state_tensor: {state_tensor.shape}')
#         return state_tensor
